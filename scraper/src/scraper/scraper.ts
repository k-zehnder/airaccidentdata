...
export const createScraper = (db: Database): Scraper => {
    ...
    const handleIndirectMapping = async (
        ...
        const indirectImageUrls =
            await parser.extractImageUrls(indirectHtmlContent);
        const firstRelevantThumbnailUrl = indirectImageUrls.find(
            (url) =>
                url.includes('File:') &&
                !url.includes('Commons-logo') &&
                !url.includes('Aviacionavion'),
        );

        if (firstRelevantThumbnailUrl) {
            const filePageUrl = firstRelevantThumbnailUrl.startsWith(
                '/wiki/File:',
            )
    ...
    const fetchImagesFromWikipedia = async (
        ...
        const searchUrl = `https://en.wikipedia.org/w/index.php?search=${encodeURIComponent(searchTerms)}`;
        ...
        try {
            ...
            const topResultUrl = `https://en.wikipedia.org${searchResults.eq(0).attr('href')}`;
        ...
        try {
            ...
            if (!highResImageUrl) {
                ...
                if (wikipediaImages.length > 0) {
                    ...
                    console.log(
                        `Using image from Wikipedia for ${searchTermsForMap}`,
                    );
                    highResImageUrl = await fetchOriginalImageUrl(
                        filePageUrl,
                        fetcher,
                    );
                }
            ...
            console.error(
                `Error fetching images for ${searchTermsForMap}: ${error}`,
            );
            return [];
        }
    };
